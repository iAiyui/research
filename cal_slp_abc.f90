...
real*4,parameter       :: tiv = 0.03!解析時間長
...
real*4,   dimension(1:ix+1, 2) :: range_y
...
!-----------------------------------------+
    ...
    do i = 1,ix-1
        ...
        call countup_no(tmpno, coordinateno, updw,2)!最大と最小を整理する
            if(updw(1) < 0 .or. updw(1) > jx .and. updw(2) < 0 .or. updw(2) > jx )then!変な場所のメモリを参照してたりした場合修正．
                updw(1) = 0
                updw(2) = 0
            end if
            ...
            range_y(i,1) = updw(1)
            range_y(i,2) = updw(2)
        !call gen_rp(21,1,'test.csv',8,real(i), range_y(i,1),range_y(i,2) )
    end do
!-----------------------------------------+
!■■■x座標をソートして挟み込む範囲の決定■■■!
!-----------------------------------------+
...
nx = ( 0 * unit_vector_x  + 1 * unit_vector_y) 
...
subroutine coo_maxmin(list, lng, updw)
    ...
    updw(1) = minval(list) !最小値
    updw(2) = maxval(list) !最大値
    return
end subroutine
